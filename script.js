...
window.onscroll = () =>{
  navBar.classList.remove("active");  
    ...
    sections.forEach((section)=>{
        ...
        //iterate over all sections and attach active class with anchor
        //element whose contents are displayed in the browser currently
        let scrollPosition = window.scrollY;
        let secTotalHeight = section.offsetHeight - 100;
        let secDistFromParent = section.offsetTop - 100;
        let id = section.getAttribute("id");
if(
    scrollPosition >= secDistFromParent &&
    scrollPosition <= secTotalHeight+secDistFromParent
    ) {
